...
class AvatarTracker(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.db = self.bot.db[
            "avatar_tracker"
        ]  # MongoDB collection for avatar tracking
        self.tracked_users: Dict[
            str, Dict[str, Any]
        ] = {}  # Dictionary to store user tracking info
    ...
    async def upload_to_file_sharing(self, image_bytes: bytes) -> Optional[str]:
        ...
        upload_url = f"{base_url}/{filename}"

        # Headers as specified in curl command
        headers = {
            "pw": password,
            "Accept": "url",
            "rand": "24",
        ...
        try:
            async with self.session.put(
                upload_url, headers=headers, data=image_bytes
            ) as response:
                response.raise_for_status()

                # Copyparty returns the URL in the response
                url = await response.text()

                if url and url.startswith("http"):
                    return url.strip()
                else:
                    print(f"Invalid URL returned: {url}")
                    return None

        except Exception as e:
            print(f"Failed to upload image to Copyparty: {e}")
            return None

    @discord.app_commands.command(
        name="setavatarchannel",
        description="Set the default channel for avatar change notifications.",
    )
    @discord.app_commands.check(is_authorized_user)
    async def set_avatar_channel(
        ...
        await interaction.response.send_message(
            f"Default avatar notifications will be sent to {channel.mention}.",
            ephemeral=True,
        )

    @discord.app_commands.command(
        name="trackavatar", description="Track a user's avatar changes."
    )
    @discord.app_commands.check(is_authorized_user)
    async def track_avatar(
        self,
        ...
        if str(user.id) in self.tracked_users:
            await interaction.response.send_message(
                f"{user.name}'s avatar is already being tracked.", ephemeral=True
            )
            return
        ...
        channel_info = (
            ...
            if channel
            else "in the default notification channel"
        )
        await interaction.response.send_message(
            f"Started tracking {user.name}'s global and server avatars {channel_info}.",
            ephemeral=True,
        )

    @discord.app_commands.command(
        name="untrackavatar", description="Stop tracking a user's avatar changes."
    )
    @discord.app_commands.check(is_authorized_user)
    async def untrack_avatar(
        self, interaction: discord.Interaction, user: discord.User
    ) -> None:
        """Remove a user from the tracked list."""
        if str(user.id) not in self.tracked_users:
            await interaction.response.send_message(
                f"{user.name} is not being tracked.", ephemeral=True
            )
            return
        ...
        self._save_data()
        await interaction.response.send_message(
            f"Stopped tracking {user.name}'s avatars.", ephemeral=True
        )

    @discord.app_commands.command(
        name="listtrackedavatars",
        description="List all tracked users and their notification channels.",
    )
    @discord.app_commands.check(is_authorized_user)
    async def list_tracked_avatars(self, interaction: discord.Interaction) -> None:
        ...
        if not self.tracked_users:
            await interaction.response.send_message(
                "No users are being tracked.", ephemeral=True
            )
            return
    ...
    async def handle_avatar_change(
        self, user: discord.User, new_avatar_url: Optional[str], avatar_type: str
    ) -> None:
        ...
        if not file_sharing_url:
            print(
                f"Failed to upload {avatar_type} avatar for user {user.id} to the file-sharing service."
            )
            return
        ...
        if not channel_id:
            print(
                "No notification channel set. Please use /setavatarchannel to configure."
            )
            return
        ...
        await channel.send(
            f"{user.name} changed their {avatar_type} avatar! Here is the new avatar: {file_sharing_url}"
        )
